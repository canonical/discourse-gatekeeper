...
def _get_repository_name(remote_url: str):
    """Get repository name.
...
def create_github_instance(access_token: typing.Any):
...
def create_pull_request(repository: Repo, github_repository: Repository, branch_name: str):
    ...
    if _check_branch_exists(repository=repository, branch_name=branch_name):
        _merge_existing_branch(
            repository=repository, branch_name=branch_name, commit_msg=ACTIONS_COMMIT_MESSAGE
        )
    else:
        _create_branch(
            repository=repository, branch_name=branch_name, commit_msg=ACTIONS_COMMIT_MESSAGE
